...
class SortieController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, EtatRepository $etatRepository, HttpClientInterface $httpClient): Response
    {
        $sortie = new Sortie();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $street = $sortie->getLieu()->getRue();
            $zipCode = $sortie->getLieu()->getVille()->getCodePostal();
            // Fetching lat lon
            $result = $httpClient->request('GET', 'https://api-adresse.data.gouv.fr/search/', [
                'query' => [
                    'q' => $street,
                    'postcode' =>$zipCode,
                ],
            ...
            if ($lat && $lon) {
                ...
                // Set coordinates
                $sortie->getLieu()->setLongitude($lon);
                $sortie->getLieu()->setLatitude($lat);
